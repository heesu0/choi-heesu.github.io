...
> Go 코드에서 C++ 코드를 호출하기 위해 Go 기본 컴파일러(gc) 대신 gccgo, gollvm을 사용하여 C++ 함수를 링킹하는 방법이 있지만, 이는 Go에서 권장하는 방법이 아니다.

### SWIG (Simple Wrapper and Interface Generator)

두번째는 [SWIG](https://swig.org/)를 사용하는 방법이다.

SWIG는 다양한 프로그래밍 언어에서 C/C++ 코드를 사용할 수 있도록 인터페이스를 지원하는 도구이다. SWIG를 사용하면 Go 코드에서 C++ 코드를 호출할 수 있다.

하지만 SWIG는 C/C++ 코드에서 Go 코드를 호출하는 방법을 지원하지 않는다.

결국 C/C++ 코드에서 Go 코드를 호출하려면 cgo를 사용해야 한다.

<br>

## Cgo를 사용하여 C/C++ 코드에서 Go 함수를 호출하는 방법

C/C++ 코드에서 Go 함수를 호출하기 위해서는 Go 함수를 C 라이브러리로 빌드해야(내보내야) 한다. C/C++ 코드는 해당 라이브러리를 링킹해서 사용할 수 있다.

### Go 함수 내보내기

Go 함수 위에 `//export` 주석을 추가하는 방법으로 Go 함수를 내보낼 수 있다.
...
Go에서는 C 함수 포인터 호출은 지원하지 않는다. 물론 [이를 우회하는 방법](#c-함수-포인터-콜백을-사용하는-방법)이 존재한다.
...
[Go에서 C를 호출하는 경우](https://github.com/golang/go/blob/master/src/runtime/cgocall.go#L117)와 달리 [C에서 Go를 호출하는 경우](https://github.com/golang/go/blob/master/src/runtime/cgocall.go#L278)에는 Go 런타임이 OS 스레드를 잠궈서 Go 스케줄러가 Go 함수 실행 중에 고루틴을 다른 OS 스레드로 이동시키는 것을 방지한다. 이를 통해 Go 런타임의 안정적으로 관리할 수 있지만 불가피한 성능 저하가 발생한다. 
