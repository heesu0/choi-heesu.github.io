...
라이브 스트리밍에서 많이 사용되는 프로토콜인 RTMP에 대해 자세히 알아보자.

> 언제나 그렇지만 확실한 스펙을 확인할 때는 [공식 문서](https://rtmp.veriskope.com/docs/spec/)를 참고하자.
...
RTMP는 청크 스트리밍을 통해 메시지를 전송하지만, 다른 전송 프로토콜을 사용해서 온전한 RTMP 메시지를 전송할 수도 있다.

따라서 온전한 RTMP 메시지 포맷부터 알아보자.
...
이제 네트워크 상에서 RTMP가 실제로 주고받는 청크 스트림에 대해 알아보자.
...
RTMP 청크는 청크 스트림 ID를 바탕으로 수신 측에서 재조립되어 온전한 메시지를 구성한다.
...
- Publish 과정에서 실제로 주고 받는 메시지
  - `C -> S` : releaseStream
  - `C -> S` : FCPublish
  - `C -> S` : createStream
  - `C <- S` : _result(createStream)
  - `C <- S` : Stream Begin
  - `C -> S` : publish
  - `C <- S` : _result(publish)
  - `C <- S` : onStatus (NetStream.Publish.Start)
  - `C -> S` : @setDataFrame (onMetaData)
  - `C -> S` : Video/Audio Data
- Unpublish 과정에서 실제로 주고 받는 메시지
  - `C → S` : FCUnpublish
  - `C → S` : deleteStream
