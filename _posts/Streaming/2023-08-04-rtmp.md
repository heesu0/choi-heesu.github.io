...
라이브 스트리밍에서 가장 대표적인 프로토콜인 RTMP에 대해 알아보자.
...
RTMP는 비디오, 오디오 및 상호작용 콘텐츠(interactive content)를 송수신하기 위해 메시지(message)를 사용한다.

RTMP에서는 메시지를 전달하는 논리적인 채널(channel)을 스트림(stream)이라고 한다. RTMP는 하나의 연결에 하나 이상의 스트림을 다중화(mulptiplexing)할 수 있다.
...
물론 최근에 등장한 스트리밍 프로토콜(e.g. SRT, WebRTC, HESP, ...)들은 RTMP보다 지연 시간이 짧지만 이 정도면 꽤 빠르다고 할 수 있다.
...
RTMP는 압축율이 높은 최신 코덱(e.g. H.265, AV1, VP9)들을 지원하지 않는다.

<br>

## RTMP 패킷 구조

앞서 RTMP는 비디오, 오디오 및 상호작용 콘텐츠를 송수신하기 위해 메시지를 사용한다고 설명했다.
...
각 메시지에 대한 자세한 정보는 [RTMP Specification Chapter 7](https://rtmp.veriskope.com/docs/spec/#7-rtmp-messages)에서 확인할 수 있다.
...
> AMF(Action Message Format) : Action Script의 객체 그래프(object graph)를 직렬화한(serialize) 바이너리 포맷으로 Adobe Flash에서 메시지를 주고 받는 목적으로도 사용된다.
...
Object에서 사용한 속성 정보가 궁금하면 [RTMP Specification Chapter 7.2](https://rtmp.veriskope.com/docs/spec/#72-command-messages)를 참고하자.

<br>

### 3. Stream (Publish or Play)

RTMP 연결이 수립된 후, 클라이언트는 메시지를 전달하는 논리적인 채널(channel)을 생성하기 위해 Command Message(`createStream`)를 서버로 전송한다.
...
이 중 클라이언트가 서버로 스트림을 전송하는 publish와, 클라이언트가 서버로 스트림을 수신받는 play의 동작 과정은 다음과 같다.
